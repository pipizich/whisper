...
class Punctuator:
    ...
    def __init__(self, model: str = "1-800-BAD-CODE/xlm-roberta_punctuation_fullstop_truecase"):
        ...
        self.max_chunk_size = 400  # Safe token limit for punctuation model

    def punctuate(self, text: str) -> str:
        """Punctuate text with support for long texts"""
        if any(p in text for p in self.ja_punctuations):
            return text
        
        # Handle long texts by chunking
        if len(text.split()) > self.max_chunk_size:
            return self._punctuate_long_text(text)
        ...
        try:
            ...
            add_log(f"Punctuation failed: {str(e)}", "WARNING")
            return text
    
    def _punctuate_long_text(self, text: str) -> str:
        """Punctuate long text by splitting into chunks"""
        words = text.split()
        chunks = []
        current_chunk = []
        ...
        for word in words:
            ...
            if len(current_chunk) >= self.max_chunk_size:
                ...
                punctuated_chunk = self.punctuate(chunk_text)
                chunks.append(punctuated_chunk)
                current_chunk = []
        ...
        # Join chunks intelligently
        result = chunks[0] if chunks else ""
        for chunk in chunks[1:]:
            # Add space if previous chunk doesn't end with punctuation
    ...
    def postprocess(self, model_outputs, **postprocess_parameters):
        ...
        for o in model_outputs:
            ...
            for c in chunks["chunks"]:
                ...
                # Punctuate individual chunks if enabled
                if postprocess_parameters["add_punctuation"] and self.punctuator:
                    c["text"] = self.punctuator.punctuate(c["text"])
                new_chunk.append(c)
...
def merge_short_segments(chunks: List[Dict], 
    ...
    current_segment = {
        ...
        "text": chunks[0]["text"],
        "speaker_id": chunks[0].get("speaker_id", None)
    }
    
    for next_chunk in chunks[1:]:
        # Check if we should merge
        duration = current_segment["timestamp"][1] - current_segment["timestamp"][0]
        gap = next_chunk["timestamp"][0] - current_segment["timestamp"][1]
        ...
        should_merge = (
            ...
            current_segment.get("speaker_id") == next_chunk.get("speaker_id") and
            not any(p in current_segment["text"] for p in ["。", "!", "?"])  # Don't merge across sentence boundaries
        )
        
        if should_merge:
            # Merge with next chunk
            current_segment["timestamp"][1] = next_chunk["timestamp"][1]
            current_segment["text"] += " " + next_chunk["text"]
        else:
            ...
            current_segment = {
                ...
                "speaker_id": next_chunk.get("speaker_id", None)
            }
...
def generate_srt(chunks: List[Dict], speaker_wise: bool = False, merge_short: bool = True) -> str:
    """Generate SRT format from chunks with optional merging"""
    if merge_short:
    ...
    try:
        if enable_diarization:
            ...
            if add_punctuation:
                ...
                # Also punctuate chunks for SRT generation
...
def create_interface():
    with gr.Blocks(title="Kotoba Whisper Subtitle Generator", theme=gr.themes.Soft()) as app:
        ...
        **Features:**
        ...
        ### 🔧 Improvements:
        - Fixed: SRT now includes punctuation when enabled
        - Fixed: Long audio files now properly punctuated
        - Fixed: SRT segments are now optimally merged for readability
        """)
